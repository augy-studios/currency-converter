...
/* Every page is also generated as plain Markdown at /<id>.md (see
   _build/generate.js) - link to it from the top of the content area. */
function addMarkdownLink() {
  const content = document.getElementById('content');
  if (!content || !window.PAGE_ID) return;

  const link = document.createElement('a');
  link.className = 'md-link';
  link.href = `/${window.PAGE_ID}.md`;
  link.innerHTML = `
  ...
  `;
  content.insertBefore(link, content.firstElementChild);
}
